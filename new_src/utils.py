...
def save_checkpoint(model, epoch, data):
	torch.save(model.state_dict(), f'../new-checkpoints/{data.name}_{IMAGE_SIZE}_{epoch}2.pth')

def save_model(model, epoch, data):
	torch.save(model, f'../new-checkpoints/{data.name}_{IMAGE_SIZE}_{PATCH_SIZE}_{epoch+1}.pth')
...
def check_accuracy(model, loader, split, device):
	...
	out = f'{split.upper()} accuracy: {100 * correct / total} %'
	print(out, "\n")	
